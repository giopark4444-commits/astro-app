...
export async function POST(request: NextRequest) {
  ...
  // memoria, ver comentario en /api/chat. Etiqueta (0023) siempre "timeline".
  let threadId: string | null = null;
  if (memoryEnabled) {
    threadId = await ensureThread(supabase, user.id, "timeline", profileId, requestedThreadId, "timeline");
